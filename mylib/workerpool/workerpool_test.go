...
import (
	"fmt"
	"github.com/phial3/mylib/errors"
	"github.com/phial3/workpool/pool"
	"testing"
	"time"
)
...
func TestWorkerPoolStart(t *testing.T) {
	wp := pool.New(10)        //Set the maximum number of threads，设置最大线程数
...
func TestWorkerPoolDoWait(t *testing.T) {
	wp := pool.New(5) //Set the maximum number of threads，设置最大线程数
	for i := 0; i < 10; i++ {
		ii := i
