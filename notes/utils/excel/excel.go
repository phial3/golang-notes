...
import (
	"fmt"
	"github.com/phial3/go-notes/utils"
	"reflect"
	"strconv"
	"strings"
	"time"
...
// NewExcelStructDefault By default, it starts from the first row and the index starts from 0
...
// SetPointerStruct ExcelStruct Struct pointer
	...
	for i, colCell := range row {
		...
		if field, ok := c.MapIndex[i]; ok {
			...
			if fields.FieldType == "time.Time" && len(colCell) > 0 {
				t, err := time.ParseInLocation(utils.LocalDateTimePattern, colCell, time.Local)
				if err == nil {
					maps[field] = t
				} else {
					//During type conversion, whether to directly prompt to report an error when an error occurs
