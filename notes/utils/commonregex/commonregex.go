...
var (
	...
	ZhCNFullRegex       = regexp.MustCompile(ZhCNFullCharPattern)
)
...
// HasZHFullChar finds all strings with full-characters in Chinese and full-characters
func HasZhFullChar(text string) bool {
	rets := match(text, ZhCNFullRegex)
	return rets != nil && len(rets) > 0
}
