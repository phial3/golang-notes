...
// 判断中文汉字
func TestHasZH_CN(t *testing.T) {
	str := "fsdg!#@#中国123"
	var pattern = regexp.MustCompile("^[\u4e00-\u9fa5]$")

	for i, ch := range str {
		//golang中string的底层是byte类型，所以单纯的for输出中文会出现乱码，这里选择for-range来输出
		if pattern.MatchString(string(ch)) {
			//判断是否为中文，如果是返回一个true，不是返回false。这俩面MatchString的参数要求是string
			//但是 for-range 返回的 value 是 rune 类型，所以需要做一个 string() 转换
			fmt.Printf("%s 字符串第 %v 个字符是中文。是“%v”字\n", str, i+1, string(ch))
		}
	}
}

// 判断是否含有中文和全角字符
func TestHasFullChar(t *testing.T) {
	str := "fs！【】g!#34：@#中（）国，《》123。"
	var pattern = regexp.MustCompile("[^\\x00-\\xff]")
	for i, ch := range str {
		if pattern.MatchString(string(ch)) {
			fmt.Printf("%s 字符串第 %v 个字符是全角字符。是“%v”\n", str, i+1, string(ch))
		}
	}
}

func TestCommonRegex_HasFullChar(t *testing.T) {
	t.Parallel()
	// assert := assert.New(t)
	...
	for _, test := range tests {
		parsed := HasZhFullChar(test)
		assert.True(t, parsed, "they should be matched")
	}
	...
	for _, test := range passedTests {
		parsed := HasZhFullChar(test)
		assert.False(t, parsed, "they should be matched")
	}
}
